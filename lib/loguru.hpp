...
LOGURU_EXPORT extern bool
    g_preamble_header; // Prepend each log start by a descriptions line with all
                       // columns name? True by default.
LOGURU_EXPORT extern bool
...
class LOGURU_EXPORT LogScopeRAII {
  public:
    ...
    LogScopeRAII(Verbosity verbosity, const char *file, unsigned line,
                 LOGURU_FORMAT_STRING_TYPE format, va_list vlist)
        LOGURU_PRINTF_LIKE(5, 0);
    LogScopeRAII(Verbosity verbosity, const char *file, unsigned line,
                 LOGURU_FORMAT_STRING_TYPE format, ...)
...
#define DEFINE_CHECK_OP_IMPL(name, op)                                         \
    ...
    inline std::string *name(const char *expr, const T1 &v1,                   \
                             const char *op_str, const T2 &v2) {               \
...
#define CHECK_WITH_INFO_S(cond, info)                                          \
    ...
    ? (void)0                                                                  \
    : loguru::Voidify() & loguru::AbortLogger("CHECK FAILED:  " info "  ",     \
                                              __FILE__, __LINE__)
