...
loguru::init to NOT set main thread name.
        * Version 2.2.0 - 2020-07-31 - Replace LOGURU_CATCH_SIGABRT with struct
SignalOptions
...
#    ifdef __COUNTER__
#        define LOGURU_ANONYMOUS_VARIABLE(str) \
            LOGURU_CONCATENATE(str, __COUNTER__)
#    else
...
#    elif defined(_MSC_VER)
#        define LOGURU_PRINTF_LIKE(fmtarg, firstvararg)
#        define LOGURU_FORMAT_STRING_TYPE \
...
void vlog(Verbosity verbosity,
          ...
          unsigned line,
          LOGURU_FORMAT_STRING_TYPE format,
          va_list) LOGURU_PRINTF_LIKE(4, 0);
...
class LOGURU_EXPORT LogScopeRAII
{
  public:
    ...
    LogScopeRAII(Verbosity verbosity,
                 ...
                 unsigned line,
                 LOGURU_FORMAT_STRING_TYPE format,
                 va_list vlist) LOGURU_PRINTF_LIKE(5, 0);
    LogScopeRAII(Verbosity verbosity,
                 const char* file,
                 unsigned line,
                 LOGURU_FORMAT_STRING_TYPE format,
...
LOGURU_EXPORT LOGURU_NORETURN void log_and_abort(
    ...
    const Args&... args)
{
    vlog_and_abort(stack_trace_skip,
                   expr,
                   file,
                   line,
                   format,
...
#    define VLOG_SCOPE_F(verbosity, ...) \
        loguru::LogScopeRAII LOGURU_ANONYMOUS_VARIABLE( \
            const error_context_RAII_) = \
            ((verbosity) > loguru::current_verbosity_cutoff()) \
            ? loguru::LogScopeRAII() \
...
#        ifndef NDEBUG
// Debug:
#            define assert(test) CHECK_WITH_INFO_F(!!(test), #test)
#        else
#            define assert(test)
#        endif
...
#        define DEFINE_CHECK_OP_IMPL(name, op) \
            ...
            inline std::string* name(const char* expr, \
                                     const T1& v1, \
                                     const char* op_str, \
                                     const T2& v2) \
            { \
...
#        define ABORT_S() \
            loguru::Voidify() \
                & loguru::AbortLogger("ABORT: ", __FILE__, __LINE__)
...
#        define CHECK_OP_S(function_name, expr1, op, expr2) \
            while (auto error_string = loguru::function_name( \
                       #expr1 " " #op " " #expr2, \
                       loguru::referenceable_value(expr1), \
                       #op, \
                       loguru::referenceable_value(expr2))) \
            loguru::AbortLogger(error_string->c_str(), __FILE__, __LINE__)

#        define CHECK_EQ_S(expr1, expr2) \
            CHECK_OP_S(check_EQ_impl, expr1, ==, expr2)
#        define CHECK_NE_S(expr1, expr2) \
            CHECK_OP_S(check_NE_impl, expr1, !=, expr2)
#        define CHECK_LE_S(expr1, expr2) \
            CHECK_OP_S(check_LE_impl, expr1, <=, expr2)
#        define CHECK_LT_S(expr1, expr2) \
            CHECK_OP_S(check_LT_impl, expr1, <, expr2)
#        define CHECK_GE_S(expr1, expr2) \
            CHECK_OP_S(check_GE_impl, expr1, >=, expr2)
#        define CHECK_GT_S(expr1, expr2) \
            CHECK_OP_S(check_GT_impl, expr1, >, expr2)
...
#            define DLOG_S(verbosity_name) \
                DVLOG_S(loguru::Verbosity_##verbosity_name)
#        endif
