...
bool add_syslog(const char *app_name, Verbosity verbosity, int facility) {
    ...
    (void)verbosity;
    (void)facility;
    VLOG_F(g_internal_verbosity, "syslog not implemented on this system. "
...
void add_stack_cleanup(const char *find_this, const char *replace_with_this) {
    if (strlen(find_this) <= strlen(replace_with_this)) {
        LOG_F(WARNING, "add_stack_cleanup: the replacement should be shorter "
                       "than the pattern!");
        return;
    }
...
#if LOGURU_PTLS_NAMES
    (void)pthread_once(&s_pthread_key_once, make_pthread_key_name);
    if (const char *name = static_cast<const char *>(
            pthread_getspecific(s_pthread_key_name))) {
        ...
        if (g_colorlogtostderr && s_terminal_has_color) {
            if (verbosity > Verbosity_WARNING) {
                fprintf(stderr, "%s%s%s%s%s%s%s%s\n", terminal_reset(),
                        terminal_dim(), message.preamble, message.indentation,
