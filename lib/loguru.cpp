...
#    pragma warning(push)
#    pragma warning(disable : 4365)  // conversion from 'X' to 'Y',
...
#        define localtime_r(a, b) \
            localtime_s(b, a)  // No localtime_r with MSVC, but arguments are
                               // swapped for localtime_s
#    else
#        include <signal.h>
...
void syslog_log(void* /*user_data*/, const Message& message)
{
    ...
    */
    ...
    syslog(
        level, "%s%s%s", message.indentation, message.prefix, message.message);
}
...
#        if defined(__APPLE__) || defined(__linux__) || defined(__sun)
        pthread_getname_np(
            this_thread, old_thread_name, sizeof(old_thread_name));
#        endif
...
bool add_file(const char* path_in, FileMode mode, Verbosity verbosity)
{
    ...
    if (!create_directories(path)) {
        LOG_F(
            ERROR, "Failed to create directories to '" LOGURU_FMT(s) "'", path);
    }
...
#    if LOGURU_WITH_FILEABS
    ...
    snprintf(
        file_abs->mode_str, sizeof(file_abs->mode_str) - 1, "%s", mode_str);
    stat(file_abs->path, &file_abs->st);
    file_abs->fp = file;
    file_abs->verbosity = verbosity;
    add_callback(
        path_in, file_log, file_abs, verbosity, file_close, file_flush);
#    else
...
#    else
    ...
    (void)facility;
    VLOG_F(g_internal_verbosity,
           "syslog not implemented on this system. "
           "Request to install syslog logging ignored.");
    return false;
#    endif
}
...
void add_stack_cleanup(const char* find_this, const char* replace_with_this)
{
    if (strlen(find_this) <= strlen(replace_with_this)) {
        LOG_F(WARNING,
              "add_stack_cleanup: the replacement should be shorter "
              "than the pattern!");
        return;
    }
...
bool remove_callback(const char* id)
{
    ...
    if (it != s_callbacks.end()) {
        ...
        LOG_F(
            ERROR, "Failed to locate callback with id '" LOGURU_FMT(s) "'", id);
        return false;
    }
}
...
#    elif LOGURU_WINTHREADS
    ...
    strncpy_s(
        thread_name_buffer(), LOGURU_THREADNAME_WIDTH + 1, name, _TRUNCATE);
                           ...
                           unsigned line)
{
    ...
    if (g_preamble_uptime && pos < out_buff_size) {
        int bytes = snprintf(
            out_buff + pos, out_buff_size - pos, "(%8.3fs) ", uptime_sec);
        if (bytes > 0) {
            pos += bytes;
        }
    }
...
AbortLogger::~AbortLogger() noexcept(false)
{
    ...
    loguru::log_and_abort(
        1, _expr, _file, _line, LOGURU_FMT(s), message.c_str());
}
...
#    ifdef _WIN32
    strncpy_s(
        with_newline + 1, buffer_size, parent_ec.c_str(), buffer_size - 2);
#    else
...
void signal_handler(int signal_number, siginfo_t*, void*)
{
    ...
    /* There are few things that are safe to do in a signal handler,
       ...
       So we first print out what happened to stderr so we're sure that gets
       out, then we do the unsafe things, like logging the stack trace.
    */
    ...
    if (s_signal_options.unsafe_signal_handler) {
        ...
        /* Now we do unsafe things. This can for example lead to deadlocks if
           the signal was triggered from the system's memory management
           functions and the code below tries to do allocations.
        */
