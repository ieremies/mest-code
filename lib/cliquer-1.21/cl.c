    ...
    if (only_weight) {
        ...
        if (unweighted) {
            clique_unweighted_find_all(
                g, min_weight, max_weight, maximal, opts);
        } else {
        ...
        if (unweighted) {
            s = clique_unweighted_find_single(
                g, min_weight, max_weight, maximal, opts);
        } else {
...
{
    printf(
        "Usage:  %s [options] <dimacs-file>\n"
        "\n"
        "<dimacs-file> is the ASCII or binary DIMACS-format file from which to "
        "read\n"
        ...
        " -m N  --min N        Search for cliques with weight at least N.  If "
        "N=0,\n"
        "                      searches for maximum weight clique (default).\n"
        " -M N  --max N        Search for cliques with weight at most N.  If "
        "N=0,\n"
        "                      no limit is imposed (default).  N being "
        "positive is\n"
        "                      incompatible with \"--min 0\" (\"--min 1\" is "
        "assumed).\n"
        " -x    --maximal      Require cliques to be maximal.\n"
        " -u    --unweighted   Assume weight 1 for all vertices.\n"
        " -0    --from-0       Number vertices 0 to n-1 instead of 1 to n when "
        "writing.\n"
        " -r F  --reorder F    Reorder with function F.  See below for "
        "details.\n"
        " -q    --quiet        Suppresses progress output.  Specifying -q "
        "twice\n"
        "                      suppresses all output except the actual "
        "result.\n"
        "\n"
        ...
        "graphs.\n"
        "      weighted-coloring     Coloring method efficient for weighted "
        "graphs.\n"
...
void read_options(int argc, char** argv)
{
    ...
    while (TRUE) {
        ...
        c = getopt_long(
            argc, argv, "aswm:M:xur:1qh", long_options, &option_index);
        ...
        switch (c) {
            ...
            default:
                fprintf(stderr,
                        "Internal bug: line %d should never "
                        "be reached!",
                        __LINE__);
                exit(-1);
        }
...
boolean record_clique_func(set_t s, graph_t* g, clique_options* opts)
{
    if (clique_count >= clique_list_size) {
        clique_list = (set_t*)realloc(clique_list,
                                      (clique_list_size + 512) * sizeof(set_t));
