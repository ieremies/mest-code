...
UNUSED_FUNCTION INLINE static set_t set_duplicate(set_t s)
{
    ...
    n = set_new(SET_MAX_SIZE(s));
    memcpy(n, s, SET_ARRAY_LENGTH(s) * sizeof(setelement));
    return n;
}
...
UNUSED_FUNCTION INLINE static set_t set_copy(set_t dest, set_t src)
{
    ...
    memset(dest + SET_ARRAY_LENGTH(src),
           0,
           ((SET_ARRAY_LENGTH(dest) - SET_ARRAY_LENGTH(src))
            * sizeof(setelement)));
    return dest;
}
 ...
 */
UNUSED_FUNCTION INLINE static set_t set_intersection(set_t res,
                                                     set_t a,
                                                     set_t b)
{
    int i, max;
